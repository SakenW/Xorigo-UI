...
export class StyleRecipeEngine {
     ...
     * 解析并应用风格配方
     */
    parseRecipe(recipeId, axisLocks) {
     ...
     */
    generateComponentTokens(roleTokens) {
        return {
            button: {
     ...
     * 生成 CSS 变量
     */
    generateCSSVariables(roleTokens, componentTokens) {
        const cssVars = {};
        // 语义令牌 CSS 变量
     ...
     */
    parseBaseAxis(base) {
        const [colorType] = base.split('-');
        return colorType;
    }
    /**
     * 解析 Accent 轴
     */
    parseAccentAxis(accent) {
     ...
     * 应用 Dark 模式非对称映射
     */
    applyDarkModeMapping(neutralScale, accentScale, mode, tone) {
        if (mode === 'light' || mode === 'hc') {
            return { neutral: neutralScale, accent: accentScale };
        }
        const toneIntensity = this.parseToneAxis(tone);
        // Dark 模式：降低彩度，微提亮度
        const adjustedNeutral = {};
     ...
     * 生成背景令牌
     */
    generateBackgroundTokens(neutralScale, mode, surface) {
        return {
     ...
     * 生成文本令牌
     */
    generateTextTokens(neutralScale, mode) {
        return {
     ...
     * 生成边框令牌
     */
    generateBorderTokens(neutralScale, mode) {
        return {
     ...
     * 生成强调色令牌
     */
    generateAccentTokens(accentScale, toneIntensity) {
        return {
     ...
     * 生成状态令牌
     */
    generateStateTokens(scales, toneIntensity) {
        return {
     ...
     * 计算对比度分数
     */
    calculateContrastScore(roleTokens) {
 ...
 * 解析配方
 */
export function parseRecipe(recipeId, axisLocks) {
    return defaultRecipeEngine.parseRecipe(recipeId, axisLocks);
}
/**
 * 验证配方可访问性
 */
export function validateRecipeAccessibility(recipe) {
