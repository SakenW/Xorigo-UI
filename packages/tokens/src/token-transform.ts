 ...
 * 令牌转换配置
 */
export interface TokenTransformConfig {
  /** CSS变量前缀 */
  prefix?: string
  /** 是否包含透明度 */
...
export class TokenTransformer {
  private config: TokenTransformConfig

  constructor(config: TokenTransformConfig = {}) {
    this.config = {
      prefix: 'xorigo',
      includeAlpha: true,
      customMappings: {},
   ...
   * 生成CSS变量映射对象
   */
  generateTokenMapping(): Record<string, string> {
   ...
   * 验证令牌路径格式
   */
  validateTokenPath(tokenPath: string): boolean {
   ...
   * 生成Tailwind配置扩展
   */
  generateTailwindExtension(): Record<string, any> {
    const mapping = this.generateTokenMapping()
 ...
 * 便捷函数
 */
export const getCSSVariables = (): string => tokenTransformer.generateCSSVariables()
export const getTokenMapping = (): Record<string, string> => tokenTransformer.generateTokenMapping()
export const getToken = (tokenPath: string): string | null => tokenTransformer.getToken(tokenPath)
