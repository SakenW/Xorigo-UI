...
export class ColorMigrator {
  // 静态属性，用于外部访问
  static colorMigrationMap = colorMigrationMap
  static semanticColorMap = semanticColorMap
  /**
   * 替换单个类名中的硬编码颜色
   */
  static replaceSingleColor(className: string): string {
    // 直接匹配颜色映射
    if (colorMigrationMap[className as keyof typeof colorMigrationMap]) {
      return colorMigrationMap[className as keyof typeof colorMigrationMap]
    }

    // 语义化映射
    if (semanticColorMap[className as keyof typeof semanticColorMap]) {
      return semanticColorMap[className as keyof typeof semanticColorMap]
    }
    ...
    if (opacityMatch) {
      ...
      const mappedColor = colorMigrationMap[color as keyof typeof colorMigrationMap]
      if (mappedColor) {
   ...
   * 替换类名字符串中的所有硬编码颜色
   */
  static replaceColors(classString: string): string {
   ...
   * 生成迁移报告
   */
  static generateMigrationReport(content: string): {
    original: string
    migrated: string
    changes: string[]
    totalChanges: number
   ...
   * 批量迁移文件内容
   */
  static migrateFileContent(content: string): string {
    // 匹配 className 属性的内容
   ...
   * 验证迁移结果
   */
  static validateMigration(original: string, migrated: string): {
    isValid: boolean
    issues: string[]
    ...
    // 检查CSS变量格式
    const invalidVariables = migrated.match(/var\(--[^)]+\)/g)?.filter(v =>
      !v.match(/^var\(--(color|spacing|font)-[a-z0-9-]+\)$/)
    )
    if (invalidVariables) {
