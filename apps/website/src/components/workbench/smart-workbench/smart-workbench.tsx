...
export function SmartWorkbench({ className }: SmartWorkbenchProps) {
  ...
  // 搜索结果 - 使用智能Grid布局算法避免空缺
  const searchResults = (() => {
    const items = [...filteredComponents]
    const cols = 3 // 目标列数

    // 如果组件数量少于等于3个，直接返回原顺序
          ...
          <AnimatePresence mode="wait">
            {searchResults.map((component, index) => {
              ...
              // 如果是两个组件，分别占据第一、二列
                ...
                gridPosition = { gridColumn: (index + 1).toString(), gridRow: '1' }
              }
              // 多组件时的正常Grid布局
              else {
                const row = Math.floor(index / cols) + 1
                const col = (index % cols) + 1
                gridPosition = { gridColumn: col.toString(), gridRow: row.toString() }
              }
...
// 更多示例请查看文档`}
            ...
            {searchResults.length === 0 && (
              <motion.div
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                className="w-full text-center py-16 col-span-full"
              >
