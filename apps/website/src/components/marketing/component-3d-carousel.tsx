...
const Component3DCarousel = () => {
  ...
  const components = [
    { name: 'Button', icon: <Box />, color: 'from-[var(--color-primary-500)] to-[var(--color-pink-500)]', desc: '灵活的按钮组件' },
    { name: 'Card', icon: <Square />, color: 'from-[var(--color-info-500)] to-[var(--color-blue-500)]', desc: '优雅的卡片容器' },
  ...
  return (
    ...
    >
            ...
            >
              ...
              >
                ...
                <div className="relative w-full h-full bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 rounded-3xl p-6 flex flex-col items-center justify-center overflow-hidden"
                     style={{
                       background: 'linear-gradient(135deg, var(--color-surface-dark) 0%, var(--color-surface-medium) 50%, var(--color-surface-dark) 100%)'
                     }}>
                  {/* 顶部装饰光线 */}
                  ...
                  {/* 图标 */}
                  <motion.div
                    className={`relative mb-4 ${isActive ? 'p-6' : 'p-4'} bg-[var(--color-surface-light)]/5 rounded-xl backdrop-blur-sm`}
                    animate={{
                  ...
                  >
                    {React.cloneElement(component.icon as React.ReactElement, {
                      className: `${isActive ? 'w-16 h-16' : 'w-12 h-12'} text-[var(--color-text-primary)] drop-shadow-lg`
                    })}
                  </motion.div>

                  {/* 组件名称 */}
                  <motion.h3
                    className={`${isActive ? 'text-2xl' : 'text-lg'} font-bold text-[var(--color-text-primary)] mb-2`}
                  >
                    {component.name}
                  </motion.h3>

                  {/* 描述文字 */}
                  <p className={`text-center ${isActive ? 'text-sm text-[var(--color-text-tertiary)]' : 'text-xs text-gray-500'}`}>
                    {component.desc}
                  </p>
                  ...
                  {isActive && (
                    <motion.div
                      ...
                      animate={{ opacity: 1, y: 0 }}
                    >
                      <div className="px-3 py-1 bg-[var(--color-surface-light)]/10 backdrop-blur-sm rounded-full border border-white/20">
                        <span className="text-xs text-[var(--color-text-primary)] font-medium">当前选中</span>
                      </div>
                    </motion.div>
                  )}
                </div>
      ...
      {/* 控制点 */}
      <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex gap-2 bg-[var(--color-surface-dark)]/40 backdrop-blur-sm px-4 py-3 rounded-full border border-white/10 z-10">
        {components.map((component, index) => {
          ...
          return (
            <motion.button
              ...
              className={`relative rounded-full transition-all ${
                isActive
                  ? 'w-8 h-2 bg-gradient-to-r from-[var(--color-primary-500)] to-cyan-500'
            ...
            >
              {isActive && (
                <motion.div
                  className="absolute inset-0 rounded-full bg-[var(--color-surface-light)]"
                  style={{ opacity: 0.3 }}
                />
              )}
            </motion.button>
      ...
      <div className="absolute left-4 top-1/2 -translate-y-1/2 z-10">
        <motion.button
          className="w-10 h-10 bg-[var(--color-surface-dark)]/60 backdrop-blur-sm border border-white/20 rounded-full flex items-center justify-center text-[var(--color-text-primary)] hover:bg-[var(--color-primary-500)]/40 transition-all"
          onClick={handlePrev}
