...
class SmartComponentGenerator:
    def __init__(self, base_path: str = None):
        ...
        self.complexity_templates = {
            "basic": {
                "files": ["index.ts", "Component.tsx", "Component.stories.tsx"],
                "test_files": ["Component.test.tsx"],
                "docs": ["README.md"]
            },
            "medium": {
                "files": ["index.ts", "Component.tsx", "Component.types.ts", "Component.utils.ts", "Component.stories.tsx"],
                "test_files": ["Component.test.tsx", "Component.utils.test.tsx"],
                "docs": ["README.md", "usage.md", "examples.md"]
            },
            "high": {
                "files": ["index.ts", "Component.tsx", "Component.types.ts", "Component.utils.ts", "Component.hooks.ts", "Component.subcomponents.tsx", "Component.stories.tsx"],
                "test_files": ["Component.test.tsx", "Component.utils.test.tsx", "Component.hooks.test.tsx", "Component.integration.test.tsx"],
                "docs": ["README.md", "usage.md", "examples.md", "api.md", "migration.md"]
            },
            "system": {
                "files": ["index.ts", "Component.tsx", "Component.types.ts", "Component.context.ts", "Component.provider.tsx", "Component.hooks.ts", "Component.config.ts", "Component.stories.tsx"],
                "test_files": ["Component.test.tsx", "Component.context.test.tsx", "Component.hooks.test.tsx", "Component.integration.test.tsx"],
                "docs": ["README.md", "usage.md", "examples.md", "api.md", "configuration.md", "migration.md"]
            }
        }
    ...
    def generate_component(self, component_name: str, custom_features: Optional[List[str]] = None):
        ...
        # 根据复杂度生成额外文件
        if complexity in ["medium", "high", "system"]:
            # 类型文件
            types_file = component_dir / f"{name_pascal}.types.ts"
            types_content = self._generate_types_content(name_pascal, features)
            self._write_file(types_file, types_content)
            files_created.append(types_file)

        if complexity in ["high", "system"]:
            # 工具文件
            utils_file = component_dir / f"{name_pascal}.utils.ts"
            utils_content = self._generate_utils_content(name_pascal, features)
        ...
        index_content = self._generate_index_content(name_pascal, complexity)
    ...
    def _generate_component_content(self, name_pascal: str, name_kebab: str, category: str, features: List[str]) -> str:
        ...
        # 根据分类添加特定导入
        if category == "form":
            imports.append("import { useFormField } from '../../hooks/use-form-field'")
        elif category == "overlays":
            imports.append("import { Portal } from '@radix-ui/react-portal'")
        elif category == "data-display":
            imports.append("import { useState, useMemo } from 'react'")
    ...
    def _generate_component_implementation(self, name_pascal: str, name_kebab: str, features: List[str], category: str) -> str:
        ...
        # 根据分类添加特定参数
        if category == "form":
            params.extend(["value", "onChange", "placeholder", "label"])
        elif category == "overlays":
            params.extend(["open", "onOpenChange", "trigger"])
        elif category == "data-display":
            params.extend(["data", "columns"])
        ...
        # 根据分类定制渲染逻辑
        render_logic = self._generate_render_logic(name_pascal, features, category)
...
export {{ {name_pascal} }}"""
    ...
    def _generate_render_logic(self, name_pascal: str, features: List[str], category: str) -> str:
        """生成渲染逻辑"""
    ...
    def _generate_variants(self, name_kebab: str, features: List[str], category: str) -> str:
        ...
        base_styles = {
            ...
            "overlays": "fixed left-[50%] top-[50%] z-50 w-full max-w-lg translate-x-[-50%] translate-y-[-50%] border bg-background p-6 shadow-lg duration-200 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[state=closed]:slide-out-to-left-1/2 data-[state=closed]:slide-out-to-top-[48%] data-[state=open]:slide-in-from-left-1/2 data-[state=open]:slide-in-from-top-[48%] sm:rounded-lg",
            ...
            "showcase": "not-prose rounded-lg border bg-muted p-6",
            "effects": "relative overflow-hidden",
            "motion": "relative",
            "system": "relative"
        }
    ...
    def _get_category_variants(self, category: str) -> Dict[str, str]:
        ...
        variants_map = {
            ...
            "form": {
                ...
                "underlined": "border-0 border-b-2 border-input rounded-none px-0 bg-transparent"
            },
            "overlays": {
...
{props_str}
    ...
    def _generate_types_content(self, name_pascal: str, features: List[str]) -> str:
        """生成类型定义内容"""
        return f"""export type {name_pascal}Variant = "default" | "primary" | "secondary"
export type {name_pascal}Size = "sm" | "md" | "lg" | "xl"
...
export interface {name_pascal}Config {{
  animated?: boolean
  theme?: "light" | "dark" | "auto"
}}
"""

    def _generate_utils_content(self, name_pascal: str, features: List[str]) -> str:
        """生成工具函数内容"""
        return f"""export const {name_pascal.lower()}Utils = {{
...
export const create{name_pascal}Config = (config: Partial<{name_pascal}Config>) => {{
  return {{
    animated: true,
    theme: 'auto',
    ...config
...
describe('{name_pascal}', () => {{
  ...
  // 功能特定测试
  {self._generate_feature_tests(name_pascal, features)}
}})"""

    def _generate_feature_tests(self, name_pascal: str, features: List[str]) -> str:
        """生成功能特定测试"""
        tests = []
  ...
  it('shows error state', () => {{
    render(<{name_pascal} error errorMessage="Error message">Error</{name_pascal}>)
...
{self._generate_feature_stories(name_pascal, features)}
"""

    def _generate_feature_stories(self, name_pascal: str, features: List[str]) -> str:
        """生成功能特定 Story"""
        stories = []
...
export const Error: Story = {{
  args: {{
    ...
    def _generate_index_content(self, name_pascal: str, complexity: str) -> str:
        """生成导出文件内容"""
        base_exports = [
            f"export {{ {name_pascal} }} from './{name_pascal}'",
            f"export type {{ {name_pascal}Props }} from './{name_pascal}'"
        ]

        if complexity in ["medium", "high", "system"]:
            base_exports.extend([
                f"export type {{ {name_pascal}Variant, {name_pascal}Size }} from './{name_pascal}.types'",
                f"export {{ {name_pascal.lower()}Utils }} from './{name_pascal}.utils'"
            ])
...
def main():
    ...
    if len(sys.argv) < 2:
        print("❌ 请提供组件名称")
        print("用法: python smart_generator.py <组件名> [功能1,功能2,...]")
        print("示例: python smart_generator.py DataTable sorting,pagination,filtering")
