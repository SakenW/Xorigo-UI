...
import { projectDetectionSystem, type DetectionContext, DetectionOperation } from './core/detection-core'
 ...
 * @returns 检测结果列表
 */
export async function detectCodeQuality(context: DetectionContext): Promise<any[]> {
  try {
    console.log(`🔍 [${AGENT_METADATA.name}] 开始检测: ${context.filePath}`)

    const results = await projectDetectionSystem.detect(context)

    if (results.length > 0) {
      console.log(`⚠️ [${AGENT_METADATA.name}] 发现 ${results.length} 个问题`)
    } else {
      console.log(`✅ [${AGENT_METADATA.name}] 未发现问题`)
    }

    return results
  } catch (error) {
    console.error(`❌ [${AGENT_METADATA.name}] 检测失败:`, error)
    return []
  }
}
...
export function getAgentStatus() {
  const stats = projectDetectionSystem.getStats()
 ...
 * 配置 Agent
 */
export function configureAgent(config: any) {
  console.log(`⚙️ [${AGENT_METADATA.name}] 配置更新:`, config)
  projectDetectionSystem.configure(config)
}
...
export function setAgentEnabled(enabled: boolean) {
  console.log(`${enabled ? '✅' : '❌'} [${AGENT_METADATA.name}] ${enabled ? '启用' : '禁用'}`)
  projectDetectionSystem.setEnabled(enabled)
}
...
export async function healthCheck() {
  ...
  const systemHealth = await projectDetectionSystem.healthCheck()
 ...
 */
export const quickDetect = detectCodeQuality
...
async function main() {
  ...
  try {
    ...
    // 构建检测上下文
    const context: DetectionContext = buildDetectionContext(options)
...
// 构建检测上下文
function buildDetectionContext(options: any): DetectionContext {
  ...
  return {
    toolInput,
    operation: options.operation as DetectionOperation,
    metadata: {
...
// 输出检测结果
function outputResults(results: any[], options: any) {
...
// 确定退出码
function determineExitCode(results: any[]): number {
  if (!results || results.length === 0) {
    return 0
  }
...
function showHelp() {
  console.log(`
Xorigo UI 代码质量检测工具 v${AGENT_METADATA.version}
