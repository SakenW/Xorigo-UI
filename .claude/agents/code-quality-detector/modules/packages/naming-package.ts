 ...
 * Packages 目录命名检测模块
 */
export const packagesNamingModule: DetectionModule = {
  id: 'naming-package',
   ...
   * 判断是否为组件文件
   */
  isComponentFile(filePath: string, fileName: string): boolean {
   ...
   * 判断是否为工具函数文件
   */
  isUtilFile(filePath: string, fileName: string): boolean {
   ...
   * 检查目录结构
   */
  async checkDirectoryStructure(relativePath: string, results: DetectionResult[]): Promise<void> {
    // 检查是否在正确的目录中
    const validDirs = [
      'src/components',
      'src/utils',
...
DetectionSystem.registerModule(packagesNamingModule)