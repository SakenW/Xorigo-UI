 ...
 * 组件分类系统检测模块
 */
export const classificationSystemModule: DetectionModule = {
   ...
   * 检查组件目录位置是否正确
   */
  async checkDirectoryPlacement(filePath: string, componentInfo: any, results: DetectionResult[]): Promise<void> {
   ...
   * 检查组件命名一致性
   */
  async checkNamingConsistency(filePath: string, componentInfo: any, results: DetectionResult[]): Promise<void> {
   ...
   * 检查 API 设计规范
   */
  async checkAPICompliance(filePath: string, componentInfo: any, content: string, results: DetectionResult[]): Promise<void> {
    const category = this.determineComponentCategory(componentInfo.name, componentInfo.usage)
   ...
   * 提取组件信息
   */
  extractComponentInfo(filePath: string, content: string): any {
   ...
   * 提取组件名称
   */
  extractComponentName(content: string): string | null {
    const patterns = [
   ...
   * 提取变体信息
   */
  extractVariants(content: string): boolean {
   ...
   * 分析组件用途
   */
  analyzeComponentUsage(content: string): string[] {
    const usage = []
   ...
   * 确定组件分类
   */
  determineComponentCategory(name: string, usage: string[]): string {
    // 基于组件名称的分类规则
   ...
   * 获取当前目录
   */
  getCurrentDirectory(filePath: string): string {
   ...
   * 获取期望的目录
   */
  getExpectedDirectory(category: string): string {
    ...
    return directoryMappings[category as keyof typeof directoryMappings] || 'ui'
  },

  /**
   * 获取命名规则
   */
  getNamingRules(category: string): any {
    const rules = {
      'base': {
        pattern: /^[A-Z][a-zA-Z]*$/,
    ...
    return rules[category as keyof typeof rules] || { pattern: /^[A-Z][a-zA-Z]*$/, suggestion: '使用 PascalCase 命名' }
  },

  /**
   * 检查是否遵循命名规则
   */
  followsNamingRules(name: string, rules: any): boolean {
   ...
   * 检查 Props 命名
   */
  checkPropsNaming(props: string[], category: string): string[] {
    const invalidProps = []
   ...
   * 获取 API 标准
   */
  getAPIStandards(category: string): any {
    const standards = {
      'base': {
    ...
    return standards[category as keyof typeof standards] || { requiredProps: [], variants: [], accessibility: false }
  },

  /**
   * 检查必需的 Props
   */
  checkRequiredProps(actualProps: string[], requiredProps: string[]): string[] {
   ...
   * 检查是否有可访问性支持
   */
  hasAccessibilitySupport(content: string): boolean {
...
DetectionSystem.registerModule(classificationSystemModule)