 ...
 * Website 架构规则检测模块
 */
export const websiteArchitectureModule: DetectionModule = {
   ...
   */
  async checkUIComponentCreation(filePath: string, content: string, results: DetectionResult[]): Promise<void> {
    // 定义常见的 UI 组件名称模式
   ...
   * 检查 UI 组件导入是否正确
   */
  async checkUIComponentImports(filePath: string, content: string, results: DetectionResult[]): Promise<void> {
    // 检查 import 语句
   ...
   * 检查是否扩展了 packages 组件
   */
  async checkComponentExtensions(filePath: string, content: string, results: DetectionResult[]): Promise<void> {
    // 检查继承语句
   ...
   * 检查样式覆盖
   */
  async checkStyleOverrides(filePath: string, content: string, results: DetectionResult[]): Promise<void> {
    // 检查可能覆盖组件样式的情况
   ...
   * 判断是否是 UI 组件名称
   */
  isUIComponentName(name: string): boolean {
   ...
   * 判断是否是允许的业务组件
   */
  isAllowedBusinessComponent(name: string, content: string, position: number): boolean {
   ...
   * 判断导入内容是否包含 UI 组件
   */
  containsUIComponents(importText: string): boolean {
    const uiKeywords = [
      'Button', 'Card', 'Input', 'Select', 'Modal', 'Dialog', 'Popover', 'Tooltip', 'Badge', 'Tag', 'Avatar', 'Icon',
   ...
   * 检查是否有直接的 UI 渲染（非组合）
   */
  hasDirectUIRendering(content: string): boolean {
...
DetectionSystem.registerModule(websiteArchitectureModule)