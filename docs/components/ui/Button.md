# Button

> UI 组件

## 概述

Button 是 TH-UI 组件库的核心组件之一，提供了现代化的设计和强大的功能。

## 安装

```bash
npm install @th-ui/core
```

## 导入

```tsx
import { Button } from '@th-ui/core'
```

## 基础用法

```tsx
function Example() {
  return (
    <Button>点击我</Button>
  )
}
```

## API 参考

### Props

| 属性 | 类型 | 默认值 | 必填 | 说明 |
|------|------|--------|------|------|
| `children` | `React.ReactNode` | - | 否 | - |
| `loading` | `boolean` | - | 否 | 加载状态 |
| `loadingText` | `string` | - | 否 | 加载时显示的文本 |
| `leftIcon` | `React.ReactNode` | - | 否 | 左侧图标 |
| `rightIcon` | `React.ReactNode` | - | 否 | 右侧图标 |
| `iconOnly` | `boolean` | - | 否 | 仅图标模式（圆形按钮） |
| `icon` | `React.ReactNode` | - | 否 | 旧版兼容 - 使用 leftIcon 替代 |
| `iconPosition` | `'left' | 'right'` | - | 否 | 旧版兼容 - 使用 leftIcon/rightIcon 替代 |

## 变体

### variant

```tsx
<Button variant="link">示例</Button>
<Button variant="neon">示例</Button>
```

## 可访问性

Button 组件遵循 WCAG 2.1 AA 标准，支持：

- 键盘导航
- 屏幕阅读器支持
- ARIA 属性标注
- 焦点管理

## 主题支持

Button 支持 TH-UI 的完整主题系统，包括：

- 亮色/暗色模式自动切换
- 10 种预设主题配色
- 设计令牌系统集成
- 动画效果配置

## TypeScript

Button 提供完整的 TypeScript 类型定义：

```tsx
interface ButtonProps {
  children?: React.ReactNode
  loading?: boolean
  loadingText?: string
  leftIcon?: React.ReactNode
  rightIcon?: React.ReactNode
  // ... 更多属性
}
```

## 相关组件

- [Avatar](./Avatar.md)
- [Badge](./Badge.md)
- [Breadcrumb](./Breadcrumb.md)

---

**版本**: 0.1.0  
**最后更新**: 2025-10-11  
**组件路径**: `packages/core/src/components/ui/Button.tsx`
